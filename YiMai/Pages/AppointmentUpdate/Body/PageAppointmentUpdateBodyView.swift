//
//  PageAppointmentUpdateBodyView.swift
//  YiMai
//
...
public class PageAppointmentUpdateBodyView: PageBodyView {
    private var PatientBasicInfoPanel: YMTouchableView? = nil
    private var PatientConditionPanel: YMTouchableView? = nil
    private var PhotoIcon = YMLayout.GetSuitableImageView("PageAppointmentUpdatePhotoIcon")
    ...
    private let SelectDoctorButtonIcon = YMLayout.GetSuitableImageView("PageAppointmentUpdateAddDoctorIcon")
    ...
    public func Reload() {
        ParseData()
        self.DrawDocCell(PageAppointmentUpdateViewController.SelectedDoctor)
    ...
    private func UpdateBasicInfo(){
        if(nil == PageAppointmentUpdateViewController.PatientBasicInfo){
            ...
            let info = PageAppointmentUpdateViewController.PatientBasicInfo!
            let name = info["name"]!
    ...
    private func UpdateCondition() {
        ...
        if("" == PageAppointmentUpdateViewController.PatientCondition){
            DrawPatientTextLabel("现病史", panel: PatientConditionPanel!, textColor: YMColors.FontGray)
        } else {
            DrawPatientTextLabel(PageAppointmentUpdateViewController.PatientCondition, panel: PatientConditionPanel!, textColor: YMColors.FontGray)
        }
    }
    ...
    private func DrawPhotoPanel() {
        ...
        let scrollLeftIcon = YMLayout.GetSuitableImageView("PageAppointmentUpdatePhotoScrollLeft")
        let scrollRightIcon = YMLayout.GetSuitableImageView("PageAppointmentUpdatePhotoScrollRight")
...
//        MorePhotoButton?.hidden = true
    ...
    public func UpdateSelectedTime() {
        if("" == PageAppointmentUpdateViewController.SelectedTime) {
            SelectTimeLabel.text = PageAppointmentUpdateBodyView.AppointmentTimeString
        } else {
            SelectTimeLabel.text = PageAppointmentUpdateViewController.SelectedTime
        }
    }
    ...
    private func DrawConfirmButton() {
        ...
        ConfirmButton?.setTitle("发送代约请求", forState: UIControlState.Normal)
