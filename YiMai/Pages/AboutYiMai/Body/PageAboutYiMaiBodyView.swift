...
public class PageAboutYiMaiBodyView: PageBodyView {
    ...
    private func DrawButtons() {
        let intro = YMLayout.GetCommonFullWidthTouchableView(
            BodyView, useObject: AboutActions!, useMethod: PageJumpActions.PageJumpToByViewSenderSel,
            label: UILabel(), text: "简介", userStringData: YMCommonStrings.CS_PAGE_YIMAI_INTRO_NAME)
        
        let contact = YMLayout.GetCommonFullWidthTouchableView(
            BodyView, useObject: AboutActions!, useMethod: PageJumpActions.PageJumpToByViewSenderSel,
            label: UILabel(), text: "联系我们", userStringData: YMCommonStrings.CS_PAGE_YIMAI_CONTACT_NAME)
