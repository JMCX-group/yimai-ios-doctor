//
//  YiMaiAddContactsFriendsStrings.swift
//  YiMai
//
//  Created by why on 16/5/26.
//  Copyright © 2016年 why. All rights reserved.
//

import Foundation

public class YiMaiAddContactsFriendsStrings {

    public static let CS_SEARCH_PLACEHOLDER = "搜索"
    
    public static let CS_DATA_KEY_USERHEAD = "head_url"
    public static let CS_DATA_KEY_NAME = "name"
    public static let CS_DATA_KEY_HOSPATIL = "hospital"
    public static let CS_DATA_KEY_DEPARTMENT = "department"
    public static let CS_DATA_KEY_JOB_TITLE = "job_title"
    public static let CS_DATA_KEY_PHONE = "phone"
    public static let CS_DATA_KEY_USER_ID = "id"
    public static let CS_DATA_KEY_FRIEND_ADDED = "is_add_friend"
    public static let CS_DATA_KEY_SMS_STATUS = "sms_status"
    
    public static let CS_LIST_TYPE_FRIENDS = "friendsList"
    public static let CS_LIST_TYPE_OTHERS = "othersList"
}